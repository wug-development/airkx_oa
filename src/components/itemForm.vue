<template>
    <a-form-item :label="item.label" :labelCol="item.labelCol || {span: 4, offset: 0}" :wrapperCol="item.wrapperCol || {span: 12, offset: 0 }" :rules="item.rules" :name="item.parentName || item.name">
        <a-select v-if="item.type === 'select'" :placeholder="'请选择'" v-model:value="modelValue" @change="onChange" :disabled="disabled" size="large" allowClear="ture">
