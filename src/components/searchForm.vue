			...
			<a-row :gutter="24">
				<a-col v-for="(item, i) in dataModel" :key="i" :span="item.span || 8" :style="{ display: i < count ? 'block' : 'none' }">
					<a-form-item :label="item.label" :rules="item.rules" :name="item.name">
						...
						<a-month-picker v-else-if="item.type==='month'" v-model:value="form[item.name]" @change="item.onChange || (() => {})" placeholder="请选择年月" :inputReadOnly="true" v-bind="item" />
						<a-range-picker v-else-if="item.type==='daterange'" v-model:value="form[item.name]" @change="item.onChange || (() => {})" :placeholder="['开始日期', '截止日期']" :inputReadOnly="true" v-bind="item" />
